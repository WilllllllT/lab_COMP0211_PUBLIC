...
#         num_states = self.n
#         num_controls = self.m
#         #use the number of states and control inputs to create the A and B matrices
#         num_outputs = self.q
#         time_step = sim.GetTimeStep()
#         v0 = cur_u[0]
#         theta0 = cur_x[2]

#         #cur_x size is (3,3) (x, y, theta) and cur_u size is (2,3) (v, omega)

#         A = np.array([[1, 0, -v0*time_step*np.sin(theta0)],
#                       [0, 1, v0*time_step*np.cos(theta0)],
#                       [0, 0, 1]])
        

        
#         B = np.array([[time_step*np.cos(theta0), 0],
#                       [time_step*np.sin(theta0), 0],
#                       [0, time_step]])


#         # linearize the system dynamics around the current state and control input
